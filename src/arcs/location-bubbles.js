...
export function buildLocationBubbles($canvas) {
    ...
    const locs = _collectLocationData();
    if (!locs) {
        $canvas.html(
    ...
    const parts = _buildBubbleSvgParts(locs, offX, offY);
    ...
    requestAnimationFrame(() => {
        ...
        const svg =
            `<svg xmlns="http://www.w3.org/2000/svg" width="${finalW}" height="${finalH}" style="display:block;">` +
            `<g transform="translate(${shiftX},${shiftY})">` + parts.join('') + `</g></svg>`;
        $canvas.html(svg);
...
function _collectLocationData() {
    ...
    for (const loc of locs) {
        ...
        loc.color = BUBBLE_COLORS[locs.indexOf(loc) % BUBBLE_COLORS.length];
    }
    return locs;
}

function _runPhysics(locs) {
    const CX = 360, CY = 280, GRAVITY = 0.014, DAMPING = 0.7, ITERS = 260;
    for (const loc of locs) {
    ...
    for (let iter = 0; iter < ITERS; iter++) {
        ...
        for (const loc of locs) {
            loc.vx *= DAMPING; loc.vy *= DAMPING;
            loc.x  += loc.vx;  loc.y  += loc.vy;
        }
    }
}

function _buildBubbleSvgParts(locs, offX, offY) {
    const parts = [];
    for (const loc of locs) {
        ...
        const fs = Math.max(10, Math.min(15, r * 0.3));
        const maxChars = Math.max(3, Math.floor((r * 1.75) / (fs * 0.58)));
        const label    = loc.label.length > maxChars ? loc.label.slice(0, maxChars - 1) + '…' : loc.label;
        ...
        parts.push(
            ...
            `<text x="${cx}" y="${labelCY}" fill="${tc}" font-size="${fs}" font-weight="600" ` +
                `text-anchor="middle" dominant-baseline="middle" ` +
                `font-family="'JetBrains Mono',monospace,sans-serif" pointer-events="none">${escHtml(label)}</text>`,
            hasCount
                ? `<text x="${cx}" y="${countCY}" fill="${tc}" font-size="${Math.max(8, Math.round(fs * 0.72))}" ` +
                  `text-anchor="middle" dominant-baseline="middle" font-family="monospace" opacity="0.70" pointer-events="none">` +
                  `×${loc.count}</text>`
                : ''
        );
    }
